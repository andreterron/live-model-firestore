import { BaseDataManager, RelationsDefinition, LiveObject, LiveList, BaseLiveObject } from 'live-model';
...
export class FirestoreDataManager extends BaseDataManager {
    ...
    async create(type: string, obj: any, options?: any): Promise<any> {
        var res;
        obj = this.normalizeObject(type, obj, options && options.references, options);
        await this.hookBefore(type, 'beforeCreate', obj, options);
        if (obj.id) {
            await this.getCollection(type, options).doc(obj.id).set(obj);
            res = obj;
        } else {
            let ref = await this.getCollection(type, options).add(obj);
            // TODO: review the need to update ids (options: generate ids OR update on the server OR user-defined option)
            res = Object.assign({id: ref.id}, obj);
        }
        await this.hookAfter(type, 'afterCreate', obj, options, res);
        return res
    }
